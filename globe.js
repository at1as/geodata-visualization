...
var Globe = function Globe(container, urls) {
  ...
  var createBlock = function(properties) {
    ...
    block.scale.z = properties.sizeZ;
    block.scale.x = properties.sizeX;
    block.scale.y = properties.sizeY;
   ...
   * @param  {Float} data.lat - latitute position
   * @param  {Float} data.lon - longtitute position
   * @param  {Float} data.sizeX - size of the block
   * @param  {Float} data.sizeY - size of the block
   * @param  {Float} data.sizeZ - size of the block
   * @param  {String} data.color - color of the block
   * @return {this}
   */
  ...
  /**
   * Returns number of blocks (data points)
   * 
   * @return {Integer}
   */
  api.blockLength = function(data) {
    return blocks.length;
  }

  /**
   * Adds height to a block, so it grows, rather than overlapping blocks
   *
   * @return {this};
   */
  api.growBlocks = function(data) {

    console.log(blocks)
    /*blocks.forEach(function(block) {

      console.log("Comparing : " + data.lat + " against " + block.lat);

      if (data.lat === block.lat && data.lon === block.lon) {

        console.log("MATCH");
        block.sizeZ = block.sizeZ + data.sizeZ;
    ...
    var location_key = data.lat + "," + data.lon;

    if (blockHash[location_key]) {
      var block = blockHash[location_key];
      console.log("Found it. Height : " + block.scale.z);

      scene.remove(block);
      block.scale.z = block.scale.z + 1;

      console.log("New Z height : " + block.scale.z)
      scene.add(block);
